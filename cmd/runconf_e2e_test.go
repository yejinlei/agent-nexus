...
func TestConfSet_WithModelsOverride(t *testing.T) {
    ...
    err := runConfSet(runConfSetOpts{
        agents: "all",
        models: "codex=gpt-5.5,claude=glm-5.2",
        branch: "main",
    })
    if err == nil {
...
func TestGetProxySource_ExplicitFlags(t *testing.T) {
    p, src, err := getProxySource("http://test.com/v1", "test-key", "", false)
    if err != nil {
...
func TestGetProxySource_URLOnly(t *testing.T) {
    _, _, err := getProxySource("http://test.com/v1", "", "", false)
    if err == nil {
...
func TestGetProxySource_AutoDetect(t *testing.T) {
    p, src, err := getProxySource("", "", "", false)
    if err != nil {
...
func TestConfSet_DBProxySource(t *testing.T) {
    ...
    if err == nil {
        ...
        if !strings.Contains(err.Error(), "数据库中未找到") && !strings.Contains(err.Error(), "not found") {
            t.Logf("conf set db-id error: %v", err)
        }
    }
}
