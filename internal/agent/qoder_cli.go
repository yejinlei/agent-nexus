...
func (w *qoderWriter) Configure(path string, p *proxy.Proxy, model string) error {
	if model == "" {
		model = "sensenova-6.7-flash-lite"
	}
	content := "# Qoder CLI Configuration - AI Proxy\n" +
		"# Qoder CLI Configuration - AI Proxy uses ACP protocol with mcpServers for provider configuration\n\n" +
		"providers:\n" +
		"  ai-proxy:\n" +
		"    type: openai_legacy\n" +
		"    base_url: \"" + p.BaseURL + "\"\n" +
...
func (w *qoderWriter) StatusModel(path string) (model, source, notes string) {
