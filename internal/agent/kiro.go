...
func (w *KiroWriter) Configure(path string, p *proxy.Proxy, model string) error {
	if model == "" { model = modelDefault(w.Name()) }
	content := "# Kiro CLI Configuration - AI Proxy`n" +
		"# Kiro CLI Configuration - AI Proxy uses ACP protocol with mcpServers for provider configuration`n`n" +
		"providers:`n" +
		"  ai-proxy:`n" +
...
func (w *KiroWriter) StatusModel(path string) (model, source, notes string) {
